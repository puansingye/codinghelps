		...
		If IsArray($DonatePixel) Then ; if Donate Button found
			...
			If $bDonateTroop Or $bDonateSpell Then
				If $ichkExtraAlphabets = 1 Then
					...
					Local $ClanString = ""
					$ClanString = getChatString(30, $DonatePixel[1] - 28, "coc-latin-cyr")
					If $ClanString = "" Then
				...
				Else
					...
					Local $ClanString = ""
					$ClanString = getChatString(30, $DonatePixel[1] - 28, "coc-latinA")
					If $ClanString = "" Then
			...
			If _Sleep($iDelayDonateCC2) Then ExitLoop
		EndIf
;~ 		$DonatePixel = _MultiPixelSearch(222, 90, 223, $y  30, 1, 1, Hex(0xFFFFFF, 6), $offColors, 20)
		...
		$Scroll = _PixelSearch(292, 40 + $bottomOffsetY, 306, 90 + $bottomOffsetY, Hex(0x509808, 6), 20)
